...
locals {
  script_js_content = templatefile(
    "${path.module}/s3_static_files/script.js.tpl",
    {
      lambda_url = aws_lambda_function_url.glue_lambda_url.function_url
    }
  )
}
resource "aws_s3_object" "script_js" {
  bucket = aws_s3_bucket.my_files.id
  key    = "script.js"
  # Using "content =" instead of "source =" because the JS is generated by Terraform:
  content      = local.script_js_content
  content_type = "application/javascript; charset=utf-8"

  # Keep etag in sync with generated content, not the template file:
  etag = md5(local.script_js_content)
}
